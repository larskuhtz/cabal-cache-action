...
function getArgs (cmd) {
  ...
  if (process.platform !== 'win32') { storepath = ""; } // FIXME the official cabal-cache version doesn't have this feature
...
async function installCabalCache() {
  const url = process.platform === 'win32'
    ? "https://kadena-cabal-cache.s3.amazonaws.com/cabal-cache/cabal-cache_x86_64_windows.tar.gz"
    : process.platform === 'darwin'
    ? "https://github.com/haskell-works/cabal-cache/releases/download/v1.0.1.8/cabal-cache_x86_64_osx.tar.gz"
    : process.platform === 'linux'
    ? "https://github.com/haskell-works/cabal-cache/releases/download/v1.0.1.8/cabal-cache_x86_64_linux.tar.gz"
    : undefined;
  ...
  const cachedPath = await tools.cacheDir(tmpPath, 'cabal-cache', '1.0.1.8');
...
async function runCabalCache(args) {
  ...
  await exec.exec(`${cabalCache} ${args.cmd} --threads 16 --archive-uri ${uri} --region ${args.region} ${args.storepath}`);
}
